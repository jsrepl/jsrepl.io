...
JSREPL offers a full Tailwind CSS experience. Unlike the official
[Tailwind Playground](https://play.tailwindcss.com/), JSREPL doesn't use the static CDN version of
Tailwind CSS. Instead, it uses the Tailwind CSS compiler to produce production-level CSS, just like
in real projects. You can define your own Tailwind config, use the `@apply` directive, and the
resulting CSS will only include used classes. Tailwind plugins are also supported, provided they're
